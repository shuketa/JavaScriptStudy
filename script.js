...
TINY.accordion=function() {
	...
	// 初期化
	// t:ネストするノードのIDタグ名 ex:nested1
	// e:ネストするノードのタグ ex:h3
	// m:
	// o:最初から開いておきたいノード
	// k:
	slider.prototype.init=function(t,e,m,o,k) {
		var a = T$(t), i=s=0, n=a.childNodes, 
		l = n.length; 
		this.s = k||0; 
		this.m = m||0;
		...
		for( i; i<l; i++ ) {
			var v = n[i];
			if( v.nodeType!=3 ) { // TEXT_NODEでない場合
				this.a[s] = {}; 
				this.a[s].h = h = T$$(e,v)[0];		// h3要素を取得
				this.a[s].c = c = T$$('div',v)[0];	// div要素を取得
				// h3要素にクリックイベント登録
				h.onclick = new Function(this.n+'.pr(0,'+s+')');
				if( o == s ) { // 最初から開いておきたい場合
				    h.className = this.s; // class属性を書き換え
				    c.style.height = 'auto'; 
				    c.d = 1
				}else{
				    c.style.height = 0; 
	...
	slider.prototype.pr=function(f,d) {
		...
		for(var i=0;i<this.l;i++) {
			...
			clearInterval(c.t);	// タイマー解除
	
			if((k!=1&&c.d==-1)&&(f==1||i==d)) {
				c.style.height=''; 
				c.m=c.offsetHeight; // 内包されたコンテンツの高さ
				c.style.height=k+'px'; 
				c.d=1; 
				h.className=this.s; 
				su(c,1)
